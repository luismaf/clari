...
#   pin a release:  ... | bash -s -- -v 0.6.0      (or: bash -s 0.6.0)
#
...
#
# Env overrides (handy for testing): CLARI_VERSION=v0.6.0 to pin a release,
...
# VERSION pins a release, e.g.  curl -fsSL <url> | bash -s -- -v 0.6.0
set -euo pipefail
...
while [ $# -gt 0 ]; do
    case "$1" in
        -v|--version)
            [ $# -ge 2 ] || { echo "option $1 needs a value (e.g. -v 0.6.0)" >&2; exit 1; }
            CLARI_VERSION="$2"
            shift 2
            ;;
